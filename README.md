...
Real-time multiplayer investment simulation game built with TanStack Start.

- **Admin** (`/admin`) — logs in with the master password, manages teams/companies/rounds, and runs the game.
- **Teams** (`/team`) — log in with a 6-char code generated by the admin, invest in companies each round.

## Env vars (`.env.local`)

```
TURSO_DATABASE_URL="..."   # libsql/Turso URL (falls back to local file:dev.db)
TURSO_AUTH_TOKEN="..."
...
npm run db:push      # push schema to DB (Turso or local file)
npm run build && node dist/server/index.mjs
```
